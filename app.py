...
def detect_language(text: str) -> str:
    """Визначення мови з посиленою евристикою для індонезійської та інших"""
    ...
    # Українська
    if any(word in text_lower for word in ["здоров", "здоров'я", "енергія", "жінки", "чоловіки", "життя", "баланс"]):
        return "uk"

    # Російська
    if any(word in text_lower for word in ["здоровье", "питание", "энергия", "женщины", "мужчины", "жизнь"]):
        return "ru"

    # Англійська
    if any(word in text_lower for word in ["health", "nutrition", "energy", "women", "men", "life", "balance"]):
        return "en"

    # Французька
    if any(word in text_lower for word in ["santé", "nutrition", "énergie", "femmes", "hommes", "vie"]):
        return "fr"

    # Іспанська
    if any(word in text_lower for word in ["salud", "nutrición", "energía", "mujeres", "hombres", "vida"]):
        return "es"

    # Італійська
    if any(word in text_lower for word in ["salute", "nutrizione", "energia", "donne", "uomini", "vita"]):
        return "it"

    # Польська
    if any(word in text_lower for word in ["zdrowie", "odżywianie", "energia", "kobiety", "mężczyźni", "życie"]):
        return "pl"

    # Нідерландська
    if any(word in text_lower for word in ["gezondheid", "voeding", "energie", "vrouwen", "mannen", "leven"]):
        return "nl"

    # Шведська
    if any(word in text_lower for word in ["hälsa", "näring", "energi", "kvinnor", "män", "liv"]):
        return "sv"

    # Португальська
    if any(word in text_lower for word in ["saúde", "nutrição", "energia", "mulheres", "homens", "vida"]):
        return "pt"

    # Румунська
    if re.search(r'[ăĂâÂîÎșȘțȚ]', text) or any(word in text_lower for word in ["cum", "să", "alegi", "vitaminele", "potrivite", "sfaturile", "nutriționistului", "pentru", "dietă", "echilibrată"]):
        return "ro"

    # Словенська
    if any(word in text_lower for word in ["zdravje", "prehrana", "energija", "ženske", "moški", "življenje"]):
        return "sl"

    # Словацька
    if any(word in text_lower for word in ["zdravie", "výživa", "energia", "ženy", "muži", "život"]):
        return "sk"

    # Індонезійська
    if any(word in text_lower for word in ["gizi", "ahli", "makan", "sehari-hari", "pola", "kesehatan", "hidup", "seimbang", "dalam", "untuk", "dan", "ini", "sangat", "penting"]):
        return "id"

    # Малайська
    if any(word in text_lower for word in ["kesihatan", "pemakanan", "tenaga", "wanita", "lelaki", "hidup"]):
        return "ms"

    # Індійська (хінді)
    if re.search(r'[हिन्दी]', text) or any(word in text_lower for word in ["स्वास्थ्य", "पोषण", "ऊर्जा", "महिलाएं", "पुरुष", "जीवन"]):
        return "hi"

    # Чеська
    if any(word in text_lower for word in ["zdraví", "výživa", "energie", "ženy", "muži", "život"]):
        return "cs"

    # Угорська
    if any(word in text_lower for word in ["egészség", "táplálkozás", "energia", "nők", "férfiak", "élet"]):
        return "hu"

    # Сербська
    if any(word in text_lower for word in ["здравље", "исхрана", "енергија", "жене", "мушкарци", "живот"]):
        return "sr"

    # Грецька
    if re.search(r'[αβγδεζηθικλμνξοπρστυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ]', text) or any(word in text_lower for word in ["υγεία", "διατροφή", "ενέργεια", "γυναίκες", "άνδρες", "ζωή"]):
        return "el"

    # Турецька
    if any(word in text_lower for word in ["sağlık", "beslenme", "enerji", "kadınlar", "erkekler", "hayat"]):
        return "tr"

    # Арабська
    if re.search(r'[عربي]', text) or any(word in text_lower for word in ["صحة", "تغذية", "طاقة", "نساء", "رجال", "حياة"]):
        return "ar"
...
def detect_language(text: str) -> str:
    ...
    # Дуже сильна евристика для індонезійської (bahasa Indonesia)
    indonesian_keywords = [
        "gizi", "ahli", "makan", "sehari-hari", "pola", "kesehatan", "hidup", "seimbang", "dalam", "untuk", "dan", "ini", "sangat", "penting",
        "pendekatan", "berbasis", "alami", "suplemen", "vitamin", "modern", "memandang", "sebagai", "bagian", "gaya", "keseimbangan",
    ...
    # Українська
    if any(word in text_lower for word in ["здоров", "здоров'я", "енергія", "жінки", "чоловіки", "життя"]):
        return "uk"
    ...
    # Англійська
    if any(word in text_lower for word in ["health", "nutrition", "energy", "women", "men", "life"]):
        return "en"
