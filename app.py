...
def detect_language(text: str) -> str:
    ...
    # Ультра-посилена для індонезійської
    ...
    try:
        ...
        if lang in ['id', 'ms']:
            return "id"
        if lang in ['ro', 'fr', 'it', 'es'] and re.search(r'[ăĂâÂîÎșȘțȚ]', text):
            return "ro"
        return lang
...
Повертай ТІЛЬКИ повний HTML з заміненим текстом, без пояснень.
Оригінал:
{original_html}
"""
        ...
        with zipfile.ZipFile(master_zip_path, 'w', zipfile.ZIP_DEFLATED) as master_zip:
            for var_num in range(1, copies_count + 1):
                ...
                for arch_idx, arch in enumerate(archive_paths):
                    ...
                    # Додаємо в головний архів ВСІ файли
